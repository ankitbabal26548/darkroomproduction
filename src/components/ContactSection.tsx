...
import { Phone, Mail, MapPin, Clock, Instagram, Facebook, Youtube, Send, Calendar, MessageCircle, Star, CheckCircle, Camera, Aperture, Film, Zap } from 'lucide-react';
export const ContactSection = () => {
  ...
  return <section ref={sectionRef} id="contact" className="relative py-20 bg-gradient-to-br from-background via-muted/30 to-accent/5 overflow-hidden">
      {/* Floating Background Elements */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute w-64 h-64 border border-accent/20 rounded-full animate-breathing opacity-40" style={{
        left: `${20 + mousePosition.x * 10}%`,
        top: `${30 + mousePosition.y * 5}%`,
        transform: 'translate(-50%, -50%)'
      }} />
        <div className="absolute w-32 h-32 bg-gradient-to-r from-accent/10 to-accent-lighter/10 rounded-lg rotate-45 animate-spin-slow opacity-30" style={{
        ...
        {/* Floating Particles */}
        {[...Array(6)].map((_, i) => <div key={i} className="absolute w-2 h-2 bg-accent/60 rounded-full animate-floating-particle opacity-70" style={{
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-8">
              ...
              <CardContent className="p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex items-center space-x-2 mb-6">
                    {[0, 1, 2].map(step => <div key={step} className={`w-3 h-3 rounded-full transition-all duration-300 ${step <= formStep ? 'bg-accent scale-110 shadow-glow' : 'bg-muted'}`} />)}
                  </div>

                  {formStep === 0 && <div className="space-y-6 animate-aperture-reveal">
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="relative group">
                          <label className="block text-sm font-medium mb-2 text-accent">Full Name *</label>
                          <Input name="name" value={formData.name} onChange={handleInputChange} placeholder="Your full name" required className="lens-focus-effect" />
                        </div>
                        <div className="relative group">
                          <label className="block text-sm font-medium mb-2 text-accent">Email Address *</label>
                          <Input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="your@email.com" required className="lens-focus-effect" />
                        </div>
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="relative group">
                          ...
                          <Input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="+91 XXXXX XXXXX" className="lens-focus-effect" />
                        </div>
                        <div className="relative group">
                          <label className="block text-sm font-medium mb-2 text-accent">Service Type</label>
                          <select name="serviceType" value={formData.serviceType} onChange={handleInputChange} className="w-full h-10 px-3 py-2 bg-background border border-input rounded-md lens-focus-effect">
                      ...
                      <Button type="button" onClick={() => setFormStep(1)} className="w-full bg-accent text-accent-foreground hover:bg-accent/90 magnetic-hover" size="lg">
                        Continue <Zap className="w-5 h-5 ml-2" />
                      </Button>
                    </div>}

                  {formStep === 1 && <div className="space-y-6 animate-aperture-reveal">
                      <div>
                        <label className="block text-sm font-medium mb-2 text-accent">Event Date</label>
                        <Input type="date" name="eventDate" value={formData.eventDate} onChange={handleInputChange} className="lens-focus-effect" />
                      </div>

                      <div>
                        <label className="block text-sm font-medium mb-2 text-accent">Tell us about your vision *</label>
                        <Textarea name="message" value={formData.message} onChange={handleInputChange} placeholder="Describe your event, preferred style, special moments you want captured, and any specific requirements..." rows={5} required className="lens-focus-effect" />
                      </div>

                      <div className="flex space-x-4">
                        <Button type="button" onClick={() => setFormStep(0)} variant="outline" className="flex-1">
                          Back
                        </Button>
                        <Button type="submit" disabled={isSubmitting} className="flex-1 bg-accent text-accent-foreground hover:bg-accent/90 magnetic-hover" size="lg">
                  ...
                  {formStep === 3 && <div className="text-center space-y-6 animate-aperture-reveal">
            ...
            <div className="grid grid-cols-2 gap-4">
              <a href="https://wa.me/919876543210" target="_blank" rel="noopener noreferrer" className="group bg-green-500/10 border border-green-500/20 rounded-lg p-4 text-center hover:bg-green-500/20 transition-all duration-300 magnetic-hover">
                <MessageCircle className="w-6 h-6 text-green-500 mx-auto mb-2 group-hover:scale-110 transition-transform" />
                <span className="text-sm font-medium">WhatsApp</span>
              </a>
              <a href="tel:+919876543210" className="group bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 text-center hover:bg-blue-500/20 transition-all duration-300 magnetic-hover">
            ...
            <Card className="darkroom-atmosphere">
              <CardContent className="p-6">
                ...
                <div className="relative h-24 overflow-hidden">
                  {testimonials.map((testimonial, index) => <div key={index} className={`absolute inset-0 transition-all duration-500 ${index === currentTestimonial ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-4'}`}>
                      <div className="text-center">
                        <div className="flex justify-center mb-2">
                          {[...Array(testimonial.rating)].map((_, i) => <Star key={i} className="w-4 h-4 text-accent fill-current" />)}
                        </div>
            ...
            <Card className="professional-badge">
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {socialLinks.map((social, index) => <a key={index} href={social.href} target="_blank" rel="noopener noreferrer" className="group text-center space-y-2 p-3 rounded-lg bg-accent-foreground/10 hover:bg-accent-foreground/20 transition-all duration-300 magnetic-hover">
