...
export const Navbar = () => {
  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-border relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map((item, index) => <div key={item.name} className="relative group">
                <a href={item.href} className={`relative px-4 py-2 text-sm font-medium transition-all duration-300 flex items-center space-x-2 ${activeSection === item.name.toLowerCase() ? 'text-accent' : 'text-foreground hover:text-accent'}`} onClick={() => setActiveSection(item.name.toLowerCase())}>
                  {/* Aperture Icon */}
                  ...
                  {/* Focus Ring Effect */}
                  <div className={`absolute inset-0 border border-accent/30 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 ${activeSection === item.name.toLowerCase() ? 'opacity-100 scale-110' : 'scale-100'}`} />
                  ...
                  <div className="absolute inset-0 grid grid-cols-3 grid-rows-3 opacity-0 group-hover:opacity-20 transition-opacity duration-300 pointer-events-none">
                    {[...Array(9)].map((_, i) => <div key={i} className="border border-accent/30" />)}
                  </div>
                </a>
          ...
          <div className="hidden lg:flex items-center space-x-4">
            <a href="tel:+91" className="flex items-center space-x-2 text-sm text-muted-foreground hover:text-accent transition-all duration-300 group">
            ...
            <a href="mailto:hello@darkroomproduction.in" className="flex items-center space-x-2 text-sm text-muted-foreground hover:text-accent transition-all duration-300 group">
          ...
          <div className="md:hidden">
            <Button variant="ghost" size="icon" onClick={() => setIsOpen(!isOpen)} className="hover:bg-accent/10 relative group">
              <div className="relative">
      ...
      <div className={`md:hidden transition-all duration-500 overflow-hidden relative ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
        ...
        <div className="px-4 pt-2 pb-4 space-y-1 relative z-10">
          {navItems.map((item, index) => <a key={item.name} href={item.href} className="flex items-center space-x-3 px-4 py-3 text-foreground hover:text-accent hover:bg-accent/10 rounded-lg transition-all duration-300 group" onClick={() => {
          setIsOpen(false);
      ...
      <div className="hidden xl:block absolute inset-0 pointer-events-none opacity-5">
        <div className="w-full h-full border-4 border-dashed border-accent/20">
          ...
          <div className="w-full h-full grid grid-cols-3 grid-rows-3">
            {[...Array(9)].map((_, i) => <div key={i} className="border border-accent/10" />)}
          </div>
        </div>
      </div>
    </nav>;
